...
FORCE_INLINE void probe_specific_action(const bool deploy) {
	...
	if(auto_manu_level_sel) {deploy ? bltouch.deploy() : bltouch.stow();}
...
bool Probe::probe_down_to_z(const float z, const feedRate_t fr_mm_s) {
  ...
  // Re-enable stealthChop if used. Disable diag1 pin on driver.
	...
	// close for pressure leveling
  #if 0//ENABLED(BLTOUCH) && DISABLED(BLTOUCH_HS_MODE)	
	if(auto_manu_level_sel){				
		SERIAL_ECHO_MSG("## stow");
...
float Probe::run_z_probe(const bool sanity_check/*=true*/) {
  ...
  auto try_to_probe = [&](PGM_P const plbl, const float &z_probe_low_point, const feedRate_t fr_mm_s, const bool scheck, const float clearance) {
    ...
    const bool probe_fail = probe_down_to_z(z_probe_low_point, fr_mm_s),            // No probe trigger?
               early_fail = 0;//(scheck && current_position.z > -offset.z + clearance); // Probe triggered too high?
  ...
  // Double-probing does a fast probe followed by a slow probe
    ...
    do_blocking_move_to_z(current_position.z + Z_CLEARANCE_MULTI_PROBE, MMM_TO_MMS(Z_PROBE_SPEED_FAST*8));

		#if ENABLED(BLTOUCH) && DISABLED(BLTOUCH_HS_MODE)
		if(auto_manu_level_sel) bltouch.stow(); // STOW in LOW SPEED MODE on trigger on every probe action			
		#endif
    ...
    const float measured_z = probes_z_sum * RECIPROCAL(MULTIPLE_PROBING);
		...
		if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("2nd Probe Z:", z2, " Discrepancy:", first_probe_z - z2);
			zDiscrepancy = first_probe_z - z2;

		//SERIAL_ECHOLNPAIR("#Leveling# Discrepancy:", zDiscrepancy);

		// Return a weighted average of the fast and slow probes
		const float measured_z = (z2 * 3.0 + first_probe_z * 2.0) * 0.2;

		//SERIAL_ECHOLNPAIR("#Leveling# Probe Z:", measured_z);
...
float Probe::probe_at_point(const float &rx, const float &ry, const ProbePtRaise raise_after/*=PROBE_PT_NONE*/, const uint8_t verbose_level/*=0*/, const bool probe_relative/*=true*/, const bool sanity_check/*=true*/) {
  ...
  if (!deploy()) measured_z = run_z_probe(sanity_check) + offset.z;
	//thermalManager.temp_bed.target = BLTOUCH_HEAT_BED_TEMP+2;
	//thermalManager.start_watching_bed();
	
  if (!isnan(measured_z)) {
    const bool big_raise = raise_after == PROBE_PT_BIG_RAISE;
    if (big_raise || raise_after == PROBE_PT_RAISE)
      do_blocking_move_to_z(current_position.z + (big_raise ? 25 : Z_CLEARANCE_BETWEEN_PROBES), MMM_TO_MMS(Z_PROBE_SPEED_FAST*8));
