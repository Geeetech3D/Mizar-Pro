...
#define EEPROM_OFFSET_POWEROFF 790
...
typedef struct {
    uint16_t X, Y, Z, X2, Y2, Z2, Z3, Z4, E0, E1, E2, E3, E4, E5, E6, E7;
} tmc_stepper_current_t;
typedef struct {
    uint32_t X, Y, Z, X2, Y2, Z2, Z3, Z4, E0, E1, E2, E3, E4, E5, E6, E7;
} tmc_hybrid_threshold_t;
typedef struct {
    int16_t X, Y, Z, X2, Y2, Z2, Z3, Z4;
} tmc_sgt_t;
typedef struct {
    bool X, Y, Z, X2, Y2, Z2, Z3, Z4, E0, E1, E2, E3, E4, E5, E6, E7;
...
//
    ...
    // BLTOUCH
    //
    bool bltouch_enable_state;
    bool bltouch_last_written_mode;

    //
    // Buzzer control
    //
    bool buzzer_enable_state;

    //
    // filament runout check
    //
    bool filament_runout_state;
...
//
#if EITHER(TOUCH_SCREEN_CALIBRATION, LVGL_TOUCH_SCREEN_CALIB)
...
#define EEPROM_START()                 \
  ...
  {                                    \
    SERIAL_ECHO_MSG("No EEPROM.");     \
    return false;                      \
  }                                    \
  int eeprom_index = EEPROM_OFFSET
#define EEPROM_FINISH()              \
  do                                 \
  {                                  \
    DISABLE_ISRS();                  \
...
//
    ...
    //
    {
        _FIELD_TEST(bltouch_enable_state);
        const bool bltouch_enable_state = auto_manu_level_sel;
        EEPROM_WRITE(bltouch_enable_state);
    ...
    //
    {
        _FIELD_TEST(buzzer_enable_state);
        const bool buzzer_enable_state = buzzer.onoff;
        EEPROM_WRITE(buzzer_enable_state);
    }
    ...
    //
    {
        _FIELD_TEST(filament_runout_state);
        const bool filament_runout_state = filament_runout_sel;
        EEPROM_WRITE(filament_runout_state);
    }
...
//
    ...
    //
    {
        ...
        const tmc_hybrid_threshold_t tmc_hybrid_threshold = {
            .X = 100, .Y = 100, .Z = 3, .X2 = 100, .Y2 = 100, .Z2 = 3, .Z3 = 3, .Z4 = 3, .E0 = 30, .E1 = 30, .E2 = 30, .E3 = 30, .E4 = 30, .E5 = 30
        };
#endif
...
#if ENABLED(LVGL_TOUCH_SCREEN_CALIB)
			EEPROM_WRITE(calibration);
#endif
...
//
        ...
        //
        {
            _FIELD_TEST(bltouch_enable_state);
            const bool &bltouch_enable_state = auto_manu_level_sel;
            EEPROM_READ(bltouch_enable_state);

            if(!validating) {
                if(leveling_is_valid()) {
                    SERIAL_ECHO_MSG("leveling is enabled");
        ...
        //
        {
            _FIELD_TEST(buzzer_enable_state);
            const bool &buzzer_enable_state = buzzer.onoff;
            EEPROM_READ(buzzer_enable_state);
        }
        ...
        //
        {
            _FIELD_TEST(filament_runout_state);
            const bool &filament_runout_state = filament_runout_sel;
            EEPROM_READ(filament_runout_state);
        }
...
#if ENABLED(LVGL_TOUCH_SCREEN_CALIB)
				_FIELD_TEST(calibration);
				EEPROM_READ(calibration);	
#endif
...
#if ENABLED(POWER_LOSS_STORAGE_EEPROM)
            plr_data_load();
#endif
        }
...
// Report the EEPROM settings
    if(!validating && (DISABLED(EEPROM_BOOT_SILENT) || IsRunning()))
        report();
#endif
...
void MarlinSettings::store_mesh(const int8_t slot)
{
    ...
    if(status)
        ...
        DEBUG_ECHOLNPAIR("Mesh saved in slot ", slot);
...
void MarlinSettings::load_mesh(const int8_t slot, void *const into /*=nullptr*/)
{
    ...
    if(status)
        ...
        DEBUG_ECHOLNPAIR("Mesh loaded from slot ", slot);
...
#if ENABLED(POWER_LOSS_STORAGE_EEPROM)
bool MarlinSettings::plr_data_load()
{
    uint16_t working_crc = 0, stored_crc = 0;
    ...
    // Power Loss Recovery
    EEPROM_READ(recovery.info);
    EEPROM_FINISH();

    if(working_crc == stored_crc) {
#if ENABLED(EEPROM_CHITCHAT)
        SERIAL_ECHO_START();
        SERIAL_ECHOPAIR("poweron recovered (", (eeprom_index - EEPROM_OFFSET_POWEROFF));
...
bool MarlinSettings::plr_data_save()
{
    uint16_t working_crc = 0;
    ...
    EEPROM_SKIP(working_crc); // Skip the checksum slot
    working_crc = 0;
    EEPROM_WRITE(recovery.info);

    const uint16_t final_crc = working_crc;
    const int eeprom_size = eeprom_index - EEPROM_OFFSET_POWEROFF;
    eeprom_index = EEPROM_OFFSET_POWEROFF;
    EEPROM_WRITE(final_crc);
    EEPROM_FINISH();

    SERIAL_ECHO_START();
    SERIAL_ECHOPAIR("Poweroff Stored (", eeprom_size);
...
#define DEFAULT_ZJERK 0
    ...
    TERN_(TOUCH_SCREEN_CALIBRATION, touch.calibration_reset());
		TERN_(LVGL_TOUCH_SCREEN_CALIB, calibration_reset());
...
#else
    probe.offset.set(dpo[X_AXIS], dpo[Y_AXIS], dpo[Z_AXIS]);
#endif
#endif
    ...
    //#endif
    auto_manu_level_sel = 1; //1 enable 3DTouch; 0 disable 3DTouch
    if(auto_manu_level_sel)
        probe.offset.set(dpo[X_AXIS], dpo[Y_AXIS], dpo[Z_AXIS]);
    else
        probe.offset.set(dpo[X_AXIS], dpo[Y_AXIS], 0);
    //
    // buzzer reset
    //
    buzzer.onoff = 1;
    ...
    //
    // filament runout reset
    //
    filament_runout_sel = 0;
...
#define CONFIG_ECHO_MSG(STR) \
  do                         \
  {                          \
    CONFIG_ECHO_START();     \
    SERIAL_ECHOLNPGM(STR);   \
  } while (0)
#define CONFIG_ECHO_HEADING(STR) config_heading(forReplay, PSTR(STR))

#if HAS_TRINAMIC_CONFIG
inline void say_M906(const bool forReplay)
{
    CONFIG_ECHO_START();
    SERIAL_ECHOPGM("  M906");
}
...
#endif
#if ENABLED(HYBRID_THRESHOLD)
inline void say_M913(const bool forReplay)
{
    CONFIG_ECHO_START();
    SERIAL_ECHOPGM("  M913");
}
...
void MarlinSettings::report(const bool forReplay)
{
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M203 X"), LINEAR_UNIT(planner.settings.max_feedrate_mm_s[X_AXIS]), SP_Y_STR, LINEAR_UNIT(planner.settings.max_feedrate_mm_s[Y_AXIS]), SP_Z_STR, LINEAR_UNIT(planner.settings.max_feedrate_mm_s[Z_AXIS])
    ...
    LOOP_L_N(i, E_STEPPERS) {
        CONFIG_ECHO_START();
        SERIAL_ECHOLNPAIR_P(
            PSTR("  M203 T"), (int)i, SP_E_STR, VOLUMETRIC_UNIT(planner.settings.max_feedrate_mm_s[E_AXIS_N(i)]));
    }
#endif
    ...
    LOOP_L_N(i, E_STEPPERS) {
        CONFIG_ECHO_START();
        SERIAL_ECHOLNPAIR_P(
            PSTR("  M201 T"), (int)i, SP_E_STR, VOLUMETRIC_UNIT(planner.settings.max_acceleration_mm_per_s2[E_AXIS_N(i)]));
    }
#endif
...
#endif
#if HAS_CLASSIC_JERK
        " X<max_x_jerk> Y<max_y_jerk> Z<max_z_jerk>" TERN_(HAS_CLASSIC_E_JERK, " E<max_e_jerk>")
#endif
    );
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
    ...
    SERIAL_ECHOLNPAIR_P(
#if IS_CARTESIAN
        PSTR("  M206 X"), LINEAR_UNIT(home_offset.x), SP_Y_STR, LINEAR_UNIT(home_offset.y), SP_Z_STR
#else
        PSTR("  M206 Z")
#endif
        ,
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M665 S"), delta_segments_per_second, SP_P_STR, scara_home_offset.a, SP_T_STR, scara_home_offset.b, SP_Z_STR, LINEAR_UNIT(scara_home_offset.z));
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M666 X"), LINEAR_UNIT(delta_endstop_adj.a), SP_Y_STR, LINEAR_UNIT(delta_endstop_adj.b), SP_Z_STR, LINEAR_UNIT(delta_endstop_adj.c));

    CONFIG_ECHO_HEADING("Delta settings: L<diagonal rod> R<radius> H<height> S<segments per sec> XYZ<tower angle trim> ABC<rod trim>");
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M665 L"), LINEAR_UNIT(delta_diagonal_rod), PSTR(" R"), LINEAR_UNIT(delta_radius), PSTR(" H"), LINEAR_UNIT(delta_height), PSTR(" S"), delta_segments_per_second, SP_X_STR, LINEAR_UNIT(delta_tower_angle_trim.a), SP_Y_STR, LINEAR_UNIT(delta_tower_angle_trim.b), SP_Z_STR, LINEAR_UNIT(delta_tower_angle_trim.c), PSTR(" A"), LINEAR_UNIT(delta_diagonal_rod_trim.a), PSTR(" B"), LINEAR_UNIT(delta_diagonal_rod_trim.b), PSTR(" C"), LINEAR_UNIT(delta_diagonal_rod_trim.c));
...
#endif
            ,
            PID_PARAM(Kp, e), PSTR(" I"), unscalePID_i(PID_PARAM(Ki, e)), PSTR(" D"), unscalePID_d(PID_PARAM(Kd, e)));
#if ENABLED(PID_EXTRUSION_SCALING)
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR(
        "  M304 P", thermalManager.temp_bed.pid.Kp, " I", unscalePID_i(thermalManager.temp_bed.pid.Ki), " D", unscalePID_d(thermalManager.temp_bed.pid.Kd));
#endif
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M207 S"), LINEAR_UNIT(fwretract.settings.retract_length), PSTR(" W"), LINEAR_UNIT(fwretract.settings.swap_retract_length), PSTR(" F"), LINEAR_UNIT(MMS_TO_MMM(fwretract.settings.retract_feedrate_mm_s)), SP_Z_STR, LINEAR_UNIT(fwretract.settings.retract_zraise));

    CONFIG_ECHO_HEADING("Recover: S<length> F<units/m>");
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR(
        "  M208 S", LINEAR_UNIT(fwretract.settings.retract_recover_extra), " W", LINEAR_UNIT(fwretract.settings.swap_retract_recover_extra), " F", LINEAR_UNIT(MMS_TO_MMM(fwretract.settings.retract_recover_feedrate_mm_s)));
    ...
    if(chop_x || chop_y || chop_z) {
        say_M569(forReplay);
        if(chop_x)
            SERIAL_ECHOPGM_P(SP_X_STR);
        if(chop_y)
            SERIAL_ECHOPGM_P(SP_Y_STR);
        if(chop_z)
            SERIAL_ECHOPGM_P(SP_Z_STR);
        SERIAL_EOL();
    }
...
#if AXIS_HAS_STEALTHCHOP(Z3)
    if(stepperZ3.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("I2 Z"), true);
    }
#endif

#if AXIS_HAS_STEALTHCHOP(Z4)
    if(stepperZ4.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("I3 Z"), true);
    }
#endif

#if AXIS_HAS_STEALTHCHOP(E0)
    if(stepperE0.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T0 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E1)
    if(stepperE1.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T1 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E2)
    if(stepperE2.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T2 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E3)
    if(stepperE3.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T3 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E4)
    if(stepperE4.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T4 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E5)
    if(stepperE5.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T5 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E6)
    if(stepperE6.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T6 E"), true);
    }
#endif
#if AXIS_HAS_STEALTHCHOP(E7)
    if(stepperE7.get_stored_stealthChop_status()) {
        say_M569(forReplay, PSTR("T7 E"), true);
    }
#endif
...
#if HAS_MOTOR_CURRENT_PWM
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M907 X"), stepper.motor_current_setting[0], SP_Z_STR, stepper.motor_current_setting[1], SP_E_STR, stepper.motor_current_setting[2]);
...
#define _ECHO_603(N)                                                                                                                    \
  ...
  {                                                                                                                                     \
    say_M603(forReplay);                                                                                                                \
    SERIAL_ECHOLNPAIR("T" STRINGIFY(N) " L", LINEAR_UNIT(fc_settings[N].load_length), " U", LINEAR_UNIT(fc_settings[N].unload_length)); \
  } while (0);
    ...
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR_P(
        PSTR("  M425 F"), backlash.get_correction(), SP_X_STR, LINEAR_UNIT(backlash.distance_mm.x), SP_Y_STR, LINEAR_UNIT(backlash.distance_mm.y), SP_Z_STR, LINEAR_UNIT(backlash.distance_mm.z)
#ifdef BACKLASH_SMOOTHING_MM
        ,
