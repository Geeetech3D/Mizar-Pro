...
static void disp_move_step()
{
	...
	else if (z_offset_step == 2)
	{
		...
		if (gCfgItems.multiple_language != 0)
		{
			lv_label_set_text(labelStep, "1.0mm");
			lv_obj_align(labelStep, buttonStep, LV_ALIGN_IN_BOTTOM_MID, 0, BUTTON_TEXT_Y_OFFSET);
		}
	}
}
...
void lv_draw_manual_level(void)
{
	...
	lv_obj_set_pos(labExtraTemp, 40, 100);
	lv_obj_set_size(labExtraTemp, 280, 0);
	...
	lv_obj_set_pos(labBedTemp, 40, 130);
	lv_obj_set_size(labBedTemp, 280, 0);
	...
	lv_obj_set_pos(level_point_info, 40, 40);
	lv_obj_set_size(level_point_info, 280, 0);
	lv_obj_set_style(level_point_info, &style_yellow_label);
	lv_label_set_align(level_point_info, LV_LABEL_ALIGN_LEFT);
	lv_label_set_text(level_point_info, leveling_menu.levelhome);

	leve_hight_value = lv_label_create(scr, NULL);
	lv_label_set_long_mode(leve_hight_value, LV_LABEL_LONG_BREAK);
	lv_obj_set_pos(leve_hight_value, 40, 70);
	lv_obj_set_size(leve_hight_value, 280, 0);
	lv_obj_set_style(leve_hight_value, &style_yellow_label);
	lv_label_set_align(leve_hight_value, LV_LABEL_ALIGN_LEFT);
