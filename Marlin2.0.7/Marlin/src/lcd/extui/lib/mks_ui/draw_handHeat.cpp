      ...
      if (event == LV_EVENT_CLICKED) {
        ...
        if (uiCfg.curTempType == 0) {
          ...
          if (uiCfg.curSprayerChoose == 0) {
            if ((int)thermalManager.temp_hotend[uiCfg.curSprayerChoose].target > (HEATER_0_MAXTEMP - HOTEND_OVERSHOOT)) {
              thermalManager.temp_hotend[uiCfg.curSprayerChoose].target = (float)HEATER_0_MAXTEMP - HOTEND_OVERSHOOT;
            }
          }
          #if !defined(SINGLENOZZLE) && HAS_MULTI_EXTRUDER
            else if ((int)thermalManager.temp_hotend[uiCfg.curSprayerChoose].target > (HEATER_1_MAXTEMP - HOTEND_OVERSHOOT)) {
              thermalManager.temp_hotend[uiCfg.curSprayerChoose].target = (float)HEATER_1_MAXTEMP - HOTEND_OVERSHOOT;
            }
          #endif
          thermalManager.start_watching_hotend(uiCfg.curSprayerChoose);
            ...
            if ((int)thermalManager.temp_bed.target > BED_MAXTEMP - BED_OVERSHOOT) {
              thermalManager.temp_bed.target = (float)BED_MAXTEMP - BED_OVERSHOOT;
            }
            thermalManager.start_watching_bed();
          }
        #endif
				preHotCurOpt = ID_P_CUS;
        disp_desire_temp();
      }
      break;
    case ID_H_DEC:
...
void lv_draw_handHeat(void) {
  ...
  lv_obj_set_pos(buttonAdd, BTN_ROW1_COL4_POS);
  lv_obj_set_pos(buttonDec, BTN_ROW1_COL1_POS);
  lv_obj_set_pos(buttoType, BTN_ROW2_COL1_POS);
  lv_obj_set_pos(buttonStep, BTN_ROW2_COL2_POS);
  lv_obj_set_pos(buttonOff, BTN_ROW2_COL3_POS);
