...
uint8_t bmp_public_buf[10 * 1024];
...
void tft_lvgl_init()
{
    ...
    lv_disp_buf_init(&disp_buf, bmp_public_buf, NULL, LV_HOR_RES_MAX * 10); /*Initialize the display buffer*/
    ...
    enc_drv.buzz_cb = buzzer.lvgl_tone;
...
bool get_point_raw(int16_t *x, int16_t *y)
{
    return touch.getRawPoint(x, y);
}

void my_feedback(lv_indev_drv_t * indev_driver, uint8_t event_id)
{
    if(LV_EVENT_CLICKED == event_id)	buzzer.lvgl_tone();
}
