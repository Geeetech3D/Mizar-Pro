...
void lv_draw_preHeat(void) {
  ...
  lv_imgbtn_set_style(buttonCool, LV_BTN_STATE_REL, &tft_style_label_rel);
	...
	lv_obj_set_pos(buttonCool, BTN_ROW2_COL3_POS);
  lv_obj_set_pos(buttonPLA, BTN_ROW2_COL1_POS);
  lv_obj_set_pos(buttonBAS, BTN_ROW2_COL2_POS);
	lv_obj_set_pos(buttonCUS, BTN_ROW1_COL4_POS);
  ...
  if (gCfgItems.multiple_language != 0) {
	...
	lv_obj_set_size(hotendTempText, 280, 0);
	...
	lv_obj_set_size(bedTempText, 280, 0);
	...
	lv_obj_set_size(preHotInfo, 280, 0);
