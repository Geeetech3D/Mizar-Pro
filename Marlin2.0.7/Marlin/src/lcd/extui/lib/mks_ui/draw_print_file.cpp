...
static lv_obj_t * scr;
extern lv_group_t*  g;
static lv_obj_t *leveling_status_info;
...
lv_style_t style_red_color_label;
lv_style_t style_green_color_label;

extern uint8_t public_buf[512];
extern char public_buf_m[100];

uint8_t sel_id = 0;
static lv_style_t style_filename_pre, style_filename_rel;

static void init_style()
{
    lv_style_copy(&style_filename_pre, &lv_style_scr);
    lv_style_copy(&style_filename_rel, &lv_style_scr);
    style_filename_pre.body.main_color = LV_COLOR_BACKGROUND;
    style_filename_pre.body.grad_color = LV_COLOR_BACKGROUND;
    style_filename_pre.text.color 		 = LV_COLOR_TEXT;
    style_filename_pre.text.sel_color  = LV_COLOR_TEXT;
    style_filename_pre.text.font			 = LV_FONT_DEFAULT;
    style_filename_rel.body.main_color = LV_COLOR_BACKGROUND;
    style_filename_rel.body.grad_color = LV_COLOR_BACKGROUND;
    style_filename_rel.text.color 		 = LV_COLOR_TEXT;
    style_filename_rel.text.sel_color  = LV_COLOR_TEXT;
    style_filename_rel.text.font			 = &TERN(HAS_SPI_FLASH_FONT, gb2312_puhui32, lv_font_roboto_22);;

    style_filename_pre.line.width        = 0;
    style_filename_rel.line.width        = 0;
    style_filename_pre.text.letter_space = 0;
    style_filename_rel.text.letter_space = 0;
    style_filename_pre.text.line_space   = 0;
    style_filename_rel.text.line_space   = 0;
}
...
static uint8_t search_file()
{
    ...
    for(uint16_t i = 0; i < fileCnt; i++) {
        if(list_file.Sd_file_cnt == list_file.Sd_file_offset) {
            ...
            if(list_file.IsFolder[valid_name_cnt] == 0) {
...
void disp_gcode_icon(uint8_t file_num)
{
    ...
    lv_obj_set_pos(buttonPageUp, OTHER_BTN_XPIEL * 3 + FILES_INTERVAL_V * 7, titleHeight + 20);
    lv_obj_set_pos(buttonPageDown, OTHER_BTN_XPIEL * 3 + FILES_INTERVAL_V * 7, titleHeight + OTHER_BTN_YPIEL + INTERVAL_H + 20);
    lv_obj_set_pos(buttonBack, OTHER_BTN_XPIEL * 3 + FILES_INTERVAL_V * 7, titleHeight + OTHER_BTN_YPIEL * 2 + INTERVAL_H * 2 + 20);
    ...
    if(IS_SD_INSERTED()) {
        for(i = 0; i < FILE_BTN_CNT; i++) {
            ...
            */
            ...
            lv_imgbtn_set_style(buttonGcode[i], LV_BTN_STATE_PR, &style_filename_pre);
            lv_imgbtn_set_style(buttonGcode[i], LV_BTN_STATE_REL, &style_filename_rel);
            ...
            if(list_file.IsFolder[i] == 1) {
                ...
                if(i < 3)
                    lv_obj_set_pos(buttonGcode[i], BTN_X_PIXEL * i + FILES_INTERVAL_V * (i + 1), titleHeight);
                else
                    lv_obj_set_pos(buttonGcode[i], BTN_X_PIXEL * (i - 3) + FILES_INTERVAL_V * ((i - 3) + 1), BTN_Y_PIXEL + INTERVAL_H + titleHeight);

                labelPageUp[i] = lv_label_create(buttonGcode[i], NULL);
                lv_obj_set_style(labelPageUp[i], &style_filename_rel);
                lv_label_set_text(labelPageUp[i], public_buf_m);
                lv_obj_align(labelPageUp[i], buttonGcode[i], LV_ALIGN_IN_BOTTOM_MID, 0, 0);
            } else {
                if(have_pre_pic((char *)list_file.file_name[i])) {
                    ...
                    if(i < 3) {
                        lv_obj_set_pos(buttonGcode[i], BTN_X_PIXEL * i + FILES_INTERVAL_V * (i + 1) + FILE_PRE_PIC_X_OFFSET, titleHeight + FILE_PRE_PIC_Y_OFFSET);
                        ...
                        lv_obj_set_pos(buttonText[i], BTN_X_PIXEL * i + FILES_INTERVAL_V * (i + 1) + FILE_PRE_PIC_X_OFFSET, titleHeight + FILE_PRE_PIC_Y_OFFSET + 100);
                        lv_obj_set_size(buttonText[i], 100, 40);
                    } else {
                        lv_obj_set_pos(buttonGcode[i], BTN_X_PIXEL * (i - 3) + FILES_INTERVAL_V * ((i - 3) + 1) + FILE_PRE_PIC_X_OFFSET, BTN_Y_PIXEL + INTERVAL_H + titleHeight + FILE_PRE_PIC_Y_OFFSET);
                        buttonText[i] = lv_btn_create(scr, NULL);
                        //lv_obj_set_event_cb(buttonText[i], event_handler);
                        ...
                        lv_btn_set_layout(buttonText[i], LV_LAYOUT_OFF);
                        //lv_obj_set_event_cb_mks(buttonText[i], event_handler,(i+10),NULL,0);
                        lv_obj_set_pos(buttonText[i], BTN_X_PIXEL * (i - 3) + FILES_INTERVAL_V * ((i - 3) + 1) + FILE_PRE_PIC_X_OFFSET, BTN_Y_PIXEL + INTERVAL_H + titleHeight + FILE_PRE_PIC_Y_OFFSET + 100);
                        lv_obj_set_size(buttonText[i], 100, 40);
                    }
                    ...
                    lv_imgbtn_set_src(buttonGcode[i], LV_BTN_STATE_PR, "F:/bmp_clear.bin");
                    if(i < 3)
                        lv_obj_set_pos(buttonGcode[i], 5+BTN_X_PIXEL * i + FILES_INTERVAL_V * (i + 1), titleHeight);
                    else
                        lv_obj_set_pos(buttonGcode[i], 5+BTN_X_PIXEL * (i - 3) + FILES_INTERVAL_V * ((i - 3) + 1), BTN_Y_PIXEL + INTERVAL_H + titleHeight);
    ...
    lv_style_copy(&style_red_color_label, &tft_style_label_rel);
    style_red_color_label.body.main_color = LV_COLOR_RED;
    style_red_color_label.body.grad_color = LV_COLOR_RED;
    lv_style_copy(&style_green_color_label, &tft_style_label_rel);
    style_green_color_label.body.main_color = LV_COLOR_GREEN;
    style_green_color_label.body.grad_color = LV_COLOR_GREEN;
    leveling_status_info = lv_label_create(scr, NULL);
    if(gCfgItems.language == LANG_SIMPLE_CHINESE)
        lv_obj_set_pos(leveling_status_info, 360, 10);
    else
        lv_obj_set_pos(leveling_status_info, 330, 10);
    lv_label_set_body_draw(leveling_status_info, true);
    lv_label_set_align(leveling_status_info, LV_LABEL_ALIGN_RIGHT);
    if(leveling_is_valid()) {
        lv_obj_set_style(leveling_status_info, &style_green_color_label);
        lv_label_set_text(leveling_status_info, leveling_menu.levelyes);
    } else {
        lv_obj_set_style(leveling_status_info, &style_red_color_label);
        lv_label_set_text(leveling_status_info, leveling_menu.levelno);
    }
