...
/**
* global value for bltouch and manual touch select
* 1= enable 3DTouch; 0 = manual Touch
*/
bool auto_manu_level_sel;

// filament runout check enable
bool filament_runout_sel;
...
void setup_powerhold()
{
    ...
    if(ENABLED(PSU_DEFAULT_OFF)) PSU_OFF();
    else PSU_ON();
#endif
}
...
{
    OUT_WRITE(STEPPER_RESET_PIN, LOW);    // Drive down to keep motor driver chips in reset
}
void enableStepperDrivers()
{
    SET_INPUT(STEPPER_RESET_PIN);    // Set to input, allowing pullups to pull the pin high
}
#endif
...
void event_probe_failure()
{
    ...
    if(gCfgItems.language == LANG_SIMPLE_CHINESE)
        kill(MSG_LCD_PROBING_FAILED_CN);
    else
#endif
...
#endif
    TERN_(TFT_LVGL_UI, uiCfg.auto_leveling_point_status = leveling_recover);
    SERIAL_ECHO_MSG("Auto Leveling Recover.");
}
...
{
#if 0//ENABLED(POWER_LOSS_RECOVERY)
    if(printJobOngoing() && recovery.outage_flag == 0)
        recovery.save(true);
#endif
...
#if HAS_TFT_LVGL_UI
    if(disp_state != ERROR_MESSAGE_UI) {
        clear_cur_ui();
...
#error "DISABLE_(DEBUG|JTAG) is not supported for the selected MCU/Board."
    ...
    SERIAL_ECHOPGM("\nDWIN handshake ");
    if(DWIN_Handshake()) SERIAL_ECHOLNPGM("ok.");
    else SERIAL_ECHOLNPGM("error.");
    ...
    if(PWM_PIN(CASE_LIGHT_PIN)) SET_PWM(CASE_LIGHT_PIN);
...
#if ENABLED(SDSUPPORT)
        card.checkautostart();
        if(card.flag.abort_sd_printing || uiCfg.stop_reprint_step == 2) {
